...
class Adsb1090Provider(BaseFlightProvider):
    ...
    async def fetch_positions(self) -> list[dict[str, Any]]:
        """Return in-range, fresh aircraft mapped to the positions-model shape.

        Used by the streaming ETL (tracked_flights + flight_positions). Applies the
        position-fix, freshness, and range gates — but NOT the movement gate, because
        the streamer throttles each aircraft on a time interval instead (so a slow or
        holding aircraft still produces a steady trail). Each returned dict carries both
        the stable identity/metadata and the latest position sample.
        """
        raw_aircraft = await self._read_feed()
        ...
        for ac in raw_aircraft:
            ...
            lat = ac.get("lat")
            lon = ac.get("lon")
            if not hex_raw or lat is None or lon is None:
                continue
            ...
            if _haversine_meters(MIA_LAT, MIA_LON, lat, lon) > max_range_m:
                continue
            ...
            rows.append(
                {
                    "hex": str(hex_raw).strip().lstrip("~").upper(),
                    ...
                    "ground_speed_knots": _to_int(ac.get("gs")),
                    "track_deg": ac.get("track"),
                    "vertical_rate_fpm": _to_int(vertical_rate),
                    "on_ground": on_ground,
                }
            )

        logger.info("ADS-B positions snapshot", received=len(raw_aircraft), in_range=len(rows))
        return rows
