...
// ── Positions model (ADS-B edge node) ────────────────────────────────

/** Parent: latest state, one row per aircraft (ICAO 24-bit hex). */
...
/** Child: append-only position trail (time-series). */
...
/** Receiver station, auto-registered from the GPS HAT. */
...
export interface Database {
  public: {
    Tables: {
      tracked_flights: {
        Row: TrackedFlight;
        Insert: Partial<TrackedFlight> & { hex: string };
        Update: Partial<TrackedFlight>;
      };
      flight_positions: {
        Row: FlightPosition;
        Insert: Partial<FlightPosition> & { hex: string; latitude: number; longitude: number };
        Update: Partial<FlightPosition>;
      };
      receiver_stations: {
        Row: ReceiverStation;
        Insert: Partial<ReceiverStation> & { station_id: string };
        Update: Partial<ReceiverStation>;
      };
      flights_current: {
        Row: Flight;
